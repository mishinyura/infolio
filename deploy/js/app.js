 ...
 */
  ...
  if (document.querySelector('.form')) {
    ...
    const nextSlide = (btn) => {
      ...
      nextBtnForm.classList.add(classLockBtn);
    };
    ...
    const prevSlide = (btn) => {
      ...
      nextBtnForm.classList.remove(classLockBtn);
    };

    const errorMessage = (input) => {
      input.nextElementSibling.innerHTML = 'Введите не менее 2 символов';
    };

    const counterLetter = (input) => {
      let count = 0;
    ...
    const formValidate = (input) => {
      if (input.target.type === 'text') {
        temp = counterLetter(input.target);
      } else if (input.target.type === 'radio') {

      } else {

      }
      console.log(input.target.nextElementSibling);

      count++;
      if (temp) {
        nextBtnForm.classList.remove(classLockBtn);
      } else {
        errorMessage(input.target);
      }
    };
    ...
    for (let i = 0; i < inputsForm.length; i++) {
      inputsForm[i].addEventListener('change', formValidate);
    }
  }
  ...
  core.use([Autoplay, Navigation, Pagination, Scrollbar, EffectCoverflow]);

  const switcher = new core('.card-content__swiper', {
    loop: true,
    navigation: {
      nextEl: '.swiper-button-next',
      prevEl: '.swiper-button-prev',
  ...
  const swiper = new core('.similar__slider', {
    direction: 'horizontal',
    loop: true,
    navigation: {
  ...
  const detailswiper = new core('.details__swiper', {
    direction: 'horizontal',
    centeredSlides: false,
    loop: false,
    // navigation: {
